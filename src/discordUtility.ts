 ...
 */
export async function getSelf(discordClient:Client, config: BotConfig): Promise<GuildMember> {
  const guild = await getGuild(discordClient, config);
...
export function setBotActivity(discordClient: Client, botActivity: string): void {
 ...
 */
export async function getGuild(discordClient: Client, config: BotConfig): Promise<Guild> {
 ...
 */
export async function setBotNickname(botNick: string, discordClient: Client, config: BotConfig): Promise<void> {
 ...
 */
export async function getVoiceChannels(discordClient: Client, config: BotConfig) {
  const guild = await getGuild(discordClient, config);
  ...
  channels = channels.filter((c) =>
    c ? self.permissionsIn(c).has(PermissionFlagsBits.ViewChannel | PermissionFlagsBits.Connect) : false
  );
 ...
 */
export async function getVoiceChannelNames(discordClient: Client, config: BotConfig): Promise<string[]> {
 ...
 */
export async function getBotMember(discordClient: Client, config: BotConfig): Promise<GuildMember> {
  const guild = await getGuild(discordClient, config);
